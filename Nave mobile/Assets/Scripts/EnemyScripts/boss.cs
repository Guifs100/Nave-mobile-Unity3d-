...
//										  this.gameObject.transform.position.y + (speed * this.gameObject.transform.position.y * Time.deltaTime));
        ...
        this.gameObject.transform.position = new Vector3(transform.position.x, Mathf.Clamp(transform.position.y, lim , lim + 3), transform.position.z);
	}
	...
	void OnTriggerEnter2D (Collider2D col){
		...
		if(col.gameObject.tag == bullet){
			...
			if(life <= 0){
                ...
                enemyManager.deadBoss = true;
                Debug.Log(enemyManager.deadBoss);
				Destroy (this.gameObject);
			}
		}
